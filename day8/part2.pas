...
var
  n, totalpaires, idx, i, numCircuits: Integer;
  dernierepaire: Integer;
  pa, pb: Integer;
begin
  n := Loadfile(nom);
  ...
  idx := 0;
  for i := 0 to n-1 do
    for pb := i+1 to n-1 do
    begin
      paires[idx].a := i;
      paires[idx].b := pb;
      paires[idx].dist := DistanceSq(points[i], points[pb]);
      Inc(idx);
    end;
  ...
  idx := 0;
  while (numCircuits > 1) and (idx < totalpaires) do
  begin
    pa := Trouve(paires[idx].a);
    pb := Trouve(paires[idx].b);
  ...
  Solve := points[paires[dernierepaire].a].x * points[paires[dernierepaire].b].x;
end;
